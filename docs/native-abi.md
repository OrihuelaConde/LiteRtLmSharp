...
> (C-API shared lib segfaults/hangs in decode on Ubuntu 24.04; static CLI works). The
> `model-tests.yml` workflow now exercises streaming + tools on linux-x64 with gemma-4-E2B-it (on
> each push, via ci.yml).
